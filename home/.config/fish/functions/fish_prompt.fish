...
function fish_prompt
    ...
    # Colors
    # Reset
    set ResetColor (set_color normal; set_color -b 000)       # Text Reset
    ...
    set -l prompt_status (__fish_print_pipestatus "[" "] " "|" $WHITE $Red $last_pipestatus)
    ...
    set Time (date +%R)
    set PathShort (pwd|sed "s="(realpath $HOME)"=~=")

    set PROMPT_START "$ResetColor$prompt_status$WHITE$Time $Red"(string split -m2 -r . (hostname -f))[1]" $Yellow$PathShort$ResetColor"
    set PROMPT_END " \n"(set_color normal)"\$ "

    set -e __CURRENT_GIT_STATUS
    set gitstatus "$__GIT_PROMPT_DIR/gitstatus.py"

    set _GIT_STATUS (command -q /usr/bin/python3 $gitstatus)
